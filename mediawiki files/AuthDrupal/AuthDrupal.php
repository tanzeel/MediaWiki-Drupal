...
$wgHooks['UserLoginComplete'][] = 'AuthDrupal_Set_Login_Cookies';
$wgHooks['UserLogout'][] = 'AuthDrupal_Set_Logout_Cookies';
...
class AuthDrupal extends AuthPlugin {
  ...
  function connectToDB() {
    ...
    if ($GLOBALS['wgAuthDrupal_UseExtDatabase']) {
      ...
      $conn = new DatabaseMysql();
      $result = $conn->open(
          $GLOBALS['wgAuthDrupal_MySQL_Host'], $GLOBALS['wgAuthDrupal_MySQL_Username'], $GLOBALS['wgAuthDrupal_MySQL_Password'], $GLOBALS['wgAuthDrupal_MySQL_Database'], 'auth_drupal_db_error_callback');
      $this->my_dbr = $conn;
    }
    else {
  ...
  function getDrupalUser($username) {
    ...
    return $dbr->selectRow($this->drupal_users_table, "*", "LCase(name)=LCase(CONVERT($qUsername USING latin1))", "AuthDrupal::getDrupalUser");
  }
  ...
  function logit($message, $severity = 0, $id = 0) {
    ...
    $dbr = & $this->getDB();
    $inRes = $dbr->insert($this->drupal_log_table, array(
      'uid' => $uid,
      'type' => 'user',
      'message' => $message,
      'severity' => $severity,
      ...
      'timestamp' => time()), "AuthDrupal::watchdog");
  }
  ...
  function getDrupalRealname($drupal_uid) {
    ...
    $fid_fname = $dbr->selectRow($fields_table, array('fid'), array('name' => $fname_field), __METHOD__);

    $fid_lname = $dbr->selectRow($fields_table, array('fid'), array('name' => $lname_field), __METHOD__);
    ...
    $fname = $dbr->selectRow($values_table, array('value'), array('fid' => $fid_fname->fid,
      'uid' => $drupal_uid), __METHOD__);

    $lname = $dbr->selectRow($values_table, array('value'), array('fid' => $fid_lname->fid,
      'uid' => $drupal_uid), __METHOD__);
...
function AuthDrupal_Set_Login_Cookies() {
  global $wgUser;
  global $wgHooks;
  global $wgAuthDrupal_ReplaceLogin;
  //print_r($wgUser); exit;
...
function AuthDrupal_Set_Logout_Cookies() {
  global $wgCookieDomain;
  $exp = time() + 2592000; // one month in seconds
  setcookie('DRXtrArgs', "", $exp, $GLOBALS['wgCookiePath'], $wgCookieDomain);
  setcookie('DRXtrArgs2', "", $exp, $GLOBALS['wgCookiePath'], $wgCookieDomain);
}
