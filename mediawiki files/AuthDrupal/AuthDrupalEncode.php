...
function authdrupal_encode($plain_string) {
  ...
  // key is unchanged
  if (empty($GLOBALS['wgAuthDrupal_security_key']) || ( 'ReplaceThisString' == $GLOBALS['wgAuthDrupal_security_key'] )) {
    return null;
  }
...
function StaticUserLogout($prefix = null, $path = null, $domain = null) {
  ...
  setcookie('DRXtrArgs', "", time() - 3600, $path, $domain);
  setcookie('DRXtrArgs2', "", time() - 3600, $path, $domain);
}
